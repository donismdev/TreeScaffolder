...
### 8.2. Before/After 뷰 동기화
- **기능**: After View에서 특정 항목을 클릭하면, Before View(현재 상태)에서도 해당 항목을 자동으로 찾아 트리를 펼치고 선택해 줍니다. 이를 통해 계획된 변경 사항이 현재 구조의 어느 위치에 해당하는지 즉시 파악할 수 있습니다.
- **제한**: 더블클릭을 통한 트리 접기/펴기 동작은 클릭 조작의 안정성을 위해 비활성화되어 있습니다. 트리를 조작하려면 왼쪽의 화살표 아이콘을 사용하십시오.

### 8.3. 안전 기능 (자동 백업)
- **덮어쓰기 자동 백업**: 최후의 방어선으로, 적용 단계에서(Dry Run 제외) 기존 파일을 덮어쓰기 직전에 원래 파일을 `{파일명}.before_overwrite`라는 이름으로 자동 변경하여 보관합니다.
- **목적**: 실수로 중요한 파일이 포함된 스캐폴딩을 적용했더라도, 사용자가 수동으로 이전 내용을 복구할 수 있도록 보장합니다.
