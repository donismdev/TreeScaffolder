...
-   **규칙**:
    ...
    -   **엄격한 들여쓰기 (Python 스타일)**:
        -   **탭(	)과 공백(Space) 혼용 금지**: 한 트리 내에서 두 방식을 섞어서 사용할 수 없습니다. 혼용 시 `TabError`가 발생합니다.
        -   **일관된 단위 사용**: 첫 번째 들여쓰기에서 사용된 단위(예: 공백 4칸 또는 공백 2칸)가 해당 트리의 표준 단위가 됩니다. 모든 들여쓰기는 이 단위의 배수여야 하며, 일치하지 않으면 `IndentationError`가 발생합니다.
    -   이름 끝에 `/` 또는 ``가 있으면 디렉토리로, 없으면 파일로 인식합니다.
